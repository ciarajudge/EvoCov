...
parser.add_option('--gapopen', type=str, help=('Pairwise alignment score for the first residue in a gap.'))
parser.add_option('--gapext', type=str, help=('Pairwise alignment score for each additional residue in a gap.'))
...
def print_usage():
    print("""\
EMBL-EBI EMBOSS water Python Client:

Pairwise sequence alignment with Water.
...
[General]
  ...
  --baseUrl             Base URL. Defaults to:
                        https://www.ebi.ac.uk/Tools/services/rest/emboss_water

Synchronous job:
  The results/errors are returned as soon as the job is finished.
  Usage: python emboss_water.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: results as an attachment

Asynchronous job:
  Use this if you want to retrieve the results at a later time. The results
  are stored for up to 24 hours.
  Usage: python emboss_water.py --asyncjob --email <your@email.com> [options...] <SeqFile|SeqID(s)>
  Returns: jobid

Check status of Asynchronous job:
  Usage: python emboss_water.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python emboss_water.py --polljob --jobid <jobId> [--outfile string]
