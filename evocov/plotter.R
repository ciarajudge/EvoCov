...
#Page 5
layout(matrix(c(1,1,2,2,3,3, 4, 5,6,6,7,7), ncol = 2, byrow = T), heights = c(1,1,4,3,1,4))
par(mar=c(0,0,0,0))
plot(1, 1, col = "white", xaxt = "n", bty = "n", yaxt = "n")
...
par(mar=c(0,0,0,0))
plot(1, 1, col = "white", xaxt = "n", bty = "n", yaxt = "n")
text(1,1,"The candidates for epitopes were selected independently of the running of this pipeline and are stored in 
     Data/candidates.txt. Each time the pipeline is run, the epitope candidates are loaded in and scored based 
     on the data generated by the pipeline, and the top scoring candidates are returned and saved in 
     Analysis/scoredepitopes.csv. The candidate epitopes were selected based on three factors: accessibility, 
     distance between amino acids and location on the spike protein. Accessibility data was obtained from the 
     Protein Data Bank Spike Trimer 7DK3, yielding 215 stretches of accessible residues.
     These stretches were filtered to only include those in the S1 subunit of the spike, as the S2 subunit is
     not displayed on the surface of the SARS-CoV-2 particle. All permutations of 2 and 3 stretches from the 
     remaining X stretches tested for the average distance between the stretches in the combinations, and these 
     permutations were filtered to give a list of all combinations of accessible amino acids with an average 
     distance of less than 15 ångströms apart. In doing this, patches of exposed amino acids that are in close 
     proximity but not necessarily near to each other on the spike protein sequence. Longer exposed stretches
     were also chopped up to allow their residues to be scored separately, in the event that a single residue on
     the edge of a stretch may for whatever reason (being highly mutable for example) drag down the score for the
     whole sequence.", cex = 1)

spikemodel <-readPNG(system.file("img","Helpers/spike.png",package="png"))
spikelabels <-readPNG(system.file("img","Helpers/spikelabels.png",package="png"))
par(mar=c(0,0,3,0))
plot(1:10,ty="n", xaxt = "n", yaxt = "n", main = "3D Model of Spike Trimer from PDB")
rasterImage(spikemodel,1,1,7,9)
par(mar=c(0,0,3,0))
plot(1:10,ty="n", xaxt = "n", yaxt = "n", main = "Labelled Accessible AAs on Spike")
rasterImage(spikelabels,1,1,7,9)
par(mar=c(0,0,0,0))
plot(1, 1, col = "white", xaxt = "n", bty = "n", yaxt = "n")
text(1,0.8,"How Epitope Candidates are Scored", cex = 1.5, font = 3)
par(mar=c(0,0,0,0))
plot(1, 1, col = "white", xaxt = "n", bty = "n", yaxt = "n")
text(1,1,"The scoring system for epitopes is still in development but for now it takes the following factors into account:
     ", cex = 1)
